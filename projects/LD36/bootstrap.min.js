var base=base||{};!function(){var a,b=!1;base.indexFunc=function(c){var d=[["tilemap/test","assets/tilemap/test.json","tilemap"],["image/example1","assets/image/example1.png","spritesheet",{frameWidth:16,frameHeight:16,pixelated:!0}],["image/player","assets/image/player.png","spritesheet",{frameWidth:16,frameHeight:16,pixelated:!0}],["gui/dialogue","assets/gui/dialogue.svg","text"],["gui/pause","assets/gui/pause.svg","text"],["audio/jump","assets/audio/jump.wav","audio"],["audio/speak","assets/audio/speak.wav","audio"],["audio/piece","assets/audio/piece.ogg","audio"]];base.loadAssets(d.concat([["scripts/app","app.min.js","script"]])),a=new Image,a.onload=base.external(function(){b=!0}),a.src="assets/image/logo.svg"},base.renderLoadingScreen=function(c,d,e){if(b){var f=document.getElementById(c);if(null===f)return;var g=f.getContext("2d");if(null===g)return;var h=g.canvas.width,i=g.canvas.height;g.fillStyle="#FFFFFF",g.fillRect(0,0,h,i),g.drawImage(a,(h-a.width)/2,(i-a.height)/2),g.fillStyle="#000000",g.beginPath(),g.rect(20,i-40,h-40,20),g.stroke(),g.beginPath(),g.rect(20,i-40,e/(e+d)*(h-40),20),g.fill()}}}();
/*
